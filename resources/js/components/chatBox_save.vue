<!--<script setup>-->
<!--import { ref } from 'vue'-->

<!--const counter = ref(0)-->
<!--</script>-->

<template>
    <main class="content">
        <div class="container p-0">
            <h1 class="mb-3 h3">Messages</h1>
            <div class="card">
                <div class="row g-0">
                    <div class="col-12 col-lg-5 col-xl-3 border-right">
                        <div class="px-4 d-none d-md-block">
                            <div class="d-flex align-items-center">
                                <div class="flex-grow-1">
                                    <input
                                        type="text"
                                        class="my-3 form-control"
                                        placeholder="Search..."
                                    />
                                </div>
                            </div>
                        </div>
                        <a
                            href="#"
                            class="border-0 list-group-item list-group-item-action"
                        >
                            <div class="float-right badge bg-success">5</div>
                            <div class="d-flex align-items-start">
                                <img
                                    src="https://bootdey.com/img/Content/avatar/avatar5.png"
                                    class="mr-1 rounded-circle"
                                    alt="Vanessa Tucker"
                                    width="40"
                                    height="40"
                                />
                                <div class="ml-3 flex-grow-1">
                                    Vanessa Tucker
                                    <div class="small">
                                        <span class="fas fa-circle chat-online"></span> Online
                                    </div>
                                </div>
                            </div>
                        </a>
                        <a
                            href="#"
                            class="border-0 list-group-item list-group-item-action"
                        >
                            <div class="float-right badge bg-success">2</div>
                            <div class="d-flex align-items-start">
                                <img
                                    src="https://bootdey.com/img/Content/avatar/avatar2.png"
                                    class="mr-1 rounded-circle"
                                    alt="William Harris"
                                    width="40"
                                    height="40"
                                />
                                <div class="ml-3 flex-grow-1">
                                    William Harris
                                    <div class="small">
                                        <span class="fas fa-circle chat-online"></span> Online
                                    </div>
                                </div>
                            </div>
                        </a>
                        <a
                            href="#"
                            class="border-0 list-group-item list-group-item-action"
                        >
                            <div class="d-flex align-items-start">
                                <img
                                    src="https://bootdey.com/img/Content/avatar/avatar3.png"
                                    class="mr-1 rounded-circle"
                                    alt="Sharon Lessman"
                                    width="40"
                                    height="40"
                                />
                                <div class="ml-3 flex-grow-1">
                                    Sharon Lessman
                                    <div class="small">
                                        <span class="fas fa-circle chat-online"></span> Online
                                    </div>
                                </div>
                            </div>
                        </a>
                        <a
                            href="#"
                            class="border-0 list-group-item list-group-item-action"
                        >
                            <div class="d-flex align-items-start">
                                <img
                                    src="https://bootdey.com/img/Content/avatar/avatar4.png"
                                    class="mr-1 rounded-circle"
                                    alt="Christina Mason"
                                    width="40"
                                    height="40"
                                />
                                <div class="ml-3 flex-grow-1">
                                    Christina Mason
                                    <div class="small">
                                        <span class="fas fa-circle chat-offline"></span> Offline
                                    </div>
                                </div>
                            </div>
                        </a>
                        <a
                            href="#"
                            class="border-0 list-group-item list-group-item-action"
                        >
                            <div class="d-flex align-items-start">
                                <img
                                    src="https://bootdey.com/img/Content/avatar/avatar5.png"
                                    class="mr-1 rounded-circle"
                                    alt="Fiona Green"
                                    width="40"
                                    height="40"
                                />
                                <div class="ml-3 flex-grow-1">
                                    Fiona Green
                                    <div class="small">
                                        <span class="fas fa-circle chat-offline"></span> Offline
                                    </div>
                                </div>
                            </div>
                        </a>
                        <a
                            href="#"
                            class="border-0 list-group-item list-group-item-action"
                        >
                            <div class="d-flex align-items-start">
                                <img
                                    src="https://bootdey.com/img/Content/avatar/avatar2.png"
                                    class="mr-1 rounded-circle"
                                    alt="Doris Wilder"
                                    width="40"
                                    height="40"
                                />
                                <div class="ml-3 flex-grow-1">
                                    Doris Wilder
                                    <div class="small">
                                        <span class="fas fa-circle chat-offline"></span> Offline
                                    </div>
                                </div>
                            </div>
                        </a>
                        <a
                            href="#"
                            class="border-0 list-group-item list-group-item-action"
                        >
                            <div class="d-flex align-items-start">
                                <img
                                    src="https://bootdey.com/img/Content/avatar/avatar4.png"
                                    class="mr-1 rounded-circle"
                                    alt="Haley Kennedy"
                                    width="40"
                                    height="40"
                                />
                                <div class="ml-3 flex-grow-1">
                                    Haley Kennedy
                                    <div class="small">
                                        <span class="fas fa-circle chat-offline"></span> Offline
                                    </div>
                                </div>
                            </div>
                        </a>
                        <a
                            href="#"
                            class="border-0 list-group-item list-group-item-action"
                        >
                            <div class="d-flex align-items-start">
                                <img
                                    src="https://bootdey.com/img/Content/avatar/avatar3.png"
                                    class="mr-1 rounded-circle"
                                    alt="Jennifer Chang"
                                    width="40"
                                    height="40"
                                />
                                <div class="ml-3 flex-grow-1">
                                    Jennifer Chang
                                    <div class="small">
                                        <span class="fas fa-circle chat-offline"></span> Offline
                                    </div>
                                </div>
                            </div>
                        </a>
                        <hr class="mt-1 mb-0 d-block d-lg-none" />
                    </div>
                    <div class="col-12 col-lg-7 col-xl-9">
                        <div class="px-4 py-2 border-bottom d-none d-lg-block">
                            <div class="py-1 d-flex align-items-center">
                                <div class="position-relative">
                                    <img
                                        src="https://bootdey.com/img/Content/avatar/avatar3.png"
                                        class="mr-1 rounded-circle"
                                        alt="Sharon Lessman"
                                        width="40"
                                        height="40"
                                    />
                                </div>
                                <div class="pl-3 flex-grow-1">
                                    <strong>Sharon Lessman</strong>
                                    <div class="text-muted small"><em>Typing...</em></div>
                                </div>
                                <div>
                                    <button class="px-3 mr-1 btn btn-primary btn-lg">
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            width="24"
                                            height="24"
                                            viewBox="0 0 24 24"
                                            fill="none"
                                            stroke="currentColor"
                                            stroke-width="2"
                                            stroke-linecap="round"
                                            stroke-linejoin="round"
                                            class="feather feather-phone feather-lg"
                                        >
                                            <path
                                                d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"
                                            ></path>
                                        </svg>
                                    </button>
                                    <button
                                        class="px-3 mr-1 btn btn-info btn-lg d-none d-md-inline-block"
                                    >
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            width="24"
                                            height="24"
                                            viewBox="0 0 24 24"
                                            fill="none"
                                            stroke="currentColor"
                                            stroke-width="2"
                                            stroke-linecap="round"
                                            stroke-linejoin="round"
                                            class="feather feather-video feather-lg"
                                        >
                                            <polygon points="23 7 16 12 23 17 23 7"></polygon>
                                            <rect
                                                x="1"
                                                y="5"
                                                width="15"
                                                height="14"
                                                rx="2"
                                                ry="2"
                                            ></rect>
                                        </svg>
                                    </button>
                                    <button class="px-3 border btn btn-light btn-lg">
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            width="24"
                                            height="24"
                                            viewBox="0 0 24 24"
                                            fill="none"
                                            stroke="currentColor"
                                            stroke-width="2"
                                            stroke-linecap="round"
                                            stroke-linejoin="round"
                                            class="feather feather-more-horizontal feather-lg"
                                        >
                                            <circle cx="12" cy="12" r="1"></circle>
                                            <circle cx="19" cy="12" r="1"></circle>
                                            <circle cx="5" cy="12" r="1"></circle>
                                        </svg>
                                    </button>
                                </div>
                            </div>
                        </div>
                        <div class="position-relative">
                            <div class="p-4 chat-messages">
                                <div class="pb-4 chat-message-right">
                                    <div>
                                        <img
                                            src="https://bootdey.com/img/Content/avatar/avatar1.png"
                                            class="mr-1 rounded-circle"
                                            alt="Chris Wood"
                                            width="40"
                                            height="40"
                                        />
                                        <div class="mt-2 text-muted small text-nowrap">
                                            2:33 am
                                        </div>
                                    </div>
                                    <div class="px-3 py-2 mr-3 rounded flex-shrink-1 bg-light">
                                        <div class="mb-1 font-weight-bold">You</div>
                                        Lorem ipsum dolor sit amet, vis erat denique in, dicunt
                                        prodesset te vix.
                                    </div>
                                </div>
                                <div class="pb-4 chat-message-left">
                                    <div>
                                        <img
                                            src="https://bootdey.com/img/Content/avatar/avatar3.png"
                                            class="mr-1 rounded-circle"
                                            alt="Sharon Lessman"
                                            width="40"
                                            height="40"
                                        />
                                        <div class="mt-2 text-muted small text-nowrap">
                                            2:34 am
                                        </div>
                                    </div>
                                    <div class="px-3 py-2 ml-3 rounded flex-shrink-1 bg-light">
                                        <div class="mb-1 font-weight-bold">Sharon Lessman</div>
                                        Sit meis deleniti eu, pri vidit meliore docendi ut, an eum
                                        erat animal commodo.
                                    </div>
                                </div>
                                <div class="mb-4 chat-message-right">
                                    <div>
                                        <img
                                            src="https://bootdey.com/img/Content/avatar/avatar1.png"
                                            class="mr-1 rounded-circle"
                                            alt="Chris Wood"
                                            width="40"
                                            height="40"
                                        />
                                        <div class="mt-2 text-muted small text-nowrap">
                                            2:35 am
                                        </div>
                                    </div>
                                    <div class="px-3 py-2 mr-3 rounded flex-shrink-1 bg-light">
                                        <div class="mb-1 font-weight-bold">You</div>
                                        Cum ea graeci tractatos.
                                    </div>
                                </div>
                                <div class="pb-4 chat-message-left">
                                    <div>
                                        <img
                                            src="https://bootdey.com/img/Content/avatar/avatar3.png"
                                            class="mr-1 rounded-circle"
                                            alt="Sharon Lessman"
                                            width="40"
                                            height="40"
                                        />
                                        <div class="mt-2 text-muted small text-nowrap">
                                            2:36 am
                                        </div>
                                    </div>
                                    <div class="px-3 py-2 ml-3 rounded flex-shrink-1 bg-light">
                                        <div class="mb-1 font-weight-bold">Sharon Lessman</div>
                                        Sed pulvinar, massa vitae interdum pulvinar, risus lectus
                                        porttitor magna, vitae commodo lectus mauris et velit.
                                        Proin ultricies placerat imperdiet. Morbi varius quam ac
                                        venenatis tempus.
                                    </div>
                                </div>
                                <div class="pb-4 chat-message-left">
                                    <div>
                                        <img
                                            src="https://bootdey.com/img/Content/avatar/avatar3.png"
                                            class="mr-1 rounded-circle"
                                            alt="Sharon Lessman"
                                            width="40"
                                            height="40"
                                        />
                                        <div class="mt-2 text-muted small text-nowrap">
                                            2:37 am
                                        </div>
                                    </div>
                                    <div class="px-3 py-2 ml-3 rounded flex-shrink-1 bg-light">
                                        <div class="mb-1 font-weight-bold">Sharon Lessman</div>
                                        Cras pulvinar, sapien id vehicula aliquet, diam velit
                                        elementum orci.
                                    </div>
                                </div>
                                <div class="mb-4 chat-message-right">
                                    <div>
                                        <img
                                            src="https://bootdey.com/img/Content/avatar/avatar1.png"
                                            class="mr-1 rounded-circle"
                                            alt="Chris Wood"
                                            width="40"
                                            height="40"
                                        />
                                        <div class="mt-2 text-muted small text-nowrap">
                                            2:38 am
                                        </div>
                                    </div>
                                    <div class="px-3 py-2 mr-3 rounded flex-shrink-1 bg-light">
                                        <div class="mb-1 font-weight-bold">You</div>
                                        Lorem ipsum dolor sit amet, vis erat denique in, dicunt
                                        prodesset te vix.
                                    </div>
                                </div>
                                <div class="pb-4 chat-message-left">
                                    <div>
                                        <img
                                            src="https://bootdey.com/img/Content/avatar/avatar3.png"
                                            class="mr-1 rounded-circle"
                                            alt="Sharon Lessman"
                                            width="40"
                                            height="40"
                                        />
                                        <div class="mt-2 text-muted small text-nowrap">
                                            2:39 am
                                        </div>
                                    </div>
                                    <div class="px-3 py-2 ml-3 rounded flex-shrink-1 bg-light">
                                        <div class="mb-1 font-weight-bold">Sharon Lessman</div>
                                        Sit meis deleniti eu, pri vidit meliore docendi ut, an eum
                                        erat animal commodo.
                                    </div>
                                </div>
                                <div class="mb-4 chat-message-right">
                                    <div>
                                        <img
                                            src="https://bootdey.com/img/Content/avatar/avatar1.png"
                                            class="mr-1 rounded-circle"
                                            alt="Chris Wood"
                                            width="40"
                                            height="40"
                                        />
                                        <div class="mt-2 text-muted small text-nowrap">
                                            2:40 am
                                        </div>
                                    </div>
                                    <div class="px-3 py-2 mr-3 rounded flex-shrink-1 bg-light">
                                        <div class="mb-1 font-weight-bold">You</div>
                                        Cum ea graeci tractatos.
                                    </div>
                                </div>
                                <div class="mb-4 chat-message-right">
                                    <div>
                                        <img
                                            src="https://bootdey.com/img/Content/avatar/avatar1.png"
                                            class="mr-1 rounded-circle"
                                            alt="Chris Wood"
                                            width="40"
                                            height="40"
                                        />
                                        <div class="mt-2 text-muted small text-nowrap">
                                            2:41 am
                                        </div>
                                    </div>
                                    <div class="px-3 py-2 mr-3 rounded flex-shrink-1 bg-light">
                                        <div class="mb-1 font-weight-bold">You</div>
                                        Morbi finibus, lorem id placerat ullamcorper, nunc enim
                                        ultrices massa, id dignissim metus urna eget purus.
                                    </div>
                                </div>
                                <div class="pb-4 chat-message-left">
                                    <div>
                                        <img
                                            src="https://bootdey.com/img/Content/avatar/avatar3.png"
                                            class="mr-1 rounded-circle"
                                            alt="Sharon Lessman"
                                            width="40"
                                            height="40"
                                        />
                                        <div class="mt-2 text-muted small text-nowrap">
                                            2:42 am
                                        </div>
                                    </div>
                                    <div class="px-3 py-2 ml-3 rounded flex-shrink-1 bg-light">
                                        <div class="mb-1 font-weight-bold">Sharon Lessman</div>
                                        Sed pulvinar, massa vitae interdum pulvinar, risus lectus
                                        porttitor magna, vitae commodo lectus mauris et velit.
                                        Proin ultricies placerat imperdiet. Morbi varius quam ac
                                        venenatis tempus.
                                    </div>
                                </div>
                                <div class="mb-4 chat-message-right">
                                    <div>
                                        <img
                                            src="https://bootdey.com/img/Content/avatar/avatar1.png"
                                            class="mr-1 rounded-circle"
                                            alt="Chris Wood"
                                            width="40"
                                            height="40"
                                        />
                                        <div class="mt-2 text-muted small text-nowrap">
                                            2:43 am
                                        </div>
                                    </div>
                                    <div class="px-3 py-2 mr-3 rounded flex-shrink-1 bg-light">
                                        <div class="mb-1 font-weight-bold">You</div>
                                        Lorem ipsum dolor sit amet, vis erat denique in, dicunt
                                        prodesset te vix.
                                    </div>
                                </div>
                                <div class="pb-4 chat-message-left">
                                    <div>
                                        <img
                                            src="https://bootdey.com/img/Content/avatar/avatar3.png"
                                            class="mr-1 rounded-circle"
                                            alt="Sharon Lessman"
                                            width="40"
                                            height="40"
                                        />
                                        <div class="mt-2 text-muted small text-nowrap">
                                            2:44 am
                                        </div>
                                    </div>
                                    <div class="px-3 py-2 ml-3 rounded flex-shrink-1 bg-light">
                                        <div class="mb-1 font-weight-bold">Sharon Lessman</div>
                                        Sit meis deleniti eu, pri vidit meliore docendi ut, an eum
                                        erat animal commodo.
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="flex-grow-0 px-4 py-3 border-top">
                            <div class="input-group">
                                <input
                                    type="text"
                                    class="form-control"
                                    placeholder="Type your message"
                                />
                                <button class="btn btn-primary" @click="connectWebSocket">Send</button>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </main>
</template>
<script>
export default {
    name: "ChatModule",
    methods: {
        // getMessages: async() => {
        //     try {
        //         const m = await axios.get(`${rootUrl}/messages`);
        //         setMessages(m.data);
        //         setTimeout(scrollToBottom, 0);
        //     } catch (err) {
        //         console.log(err.message);
        //     }
        // }
    },
    mounted() {
        const connectWebSocket = () => {
            window.Echo.private(webSocketChannel)
                .listen('GotMessage', async (e) => {
                    alert(e.message)
                    // await getMessages();
                });
        }
    }
}
</script>
